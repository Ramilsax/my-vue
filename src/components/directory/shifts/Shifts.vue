                ...
                <v-card>
                    <v-card-title>
                        <v-layout align-center justify-space-between row fill-height>
                            <v-flex xs6>
                                <edit-shift-dialog class="left" button-text="Добавить"></edit-shift-dialog>
                            </v-flex>
                            <v-flex xs3>
                                <v-text-field
                                        v-model="search"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            onRowClick(item) {
                this.$store.commit('setShift', new Shift(item.id, item.name, item.notes, item.department, item.shift));
                this.$store.commit('openShiftEditDialog', true)
            }
        },
        //Вызывется, когда страница отображается
        mounted() {
            //Загружаем смены из БД
            this.$store.dispatch('loadShifts', this.$http);
        }
    }
</script>
